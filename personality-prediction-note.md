...
# 10. Recent Trends in Deep Learning Based Personality Detection

[paper](./finished/10.pdf)	[translation](./finished/10zh.pdf)
...
1. 从预训练模型提取出来的词向量 可以 考虑降维度，然后利用双向GRU再从原文中提取一次，做两种词向量的融合
2. 思考与LIWC , senticnet等特征结合的可能
3. 是否可以利用词性标注的特征来融合？ POS tag vector
4. 已有视觉 + 音频的双模态，可以考虑把音频转成文字，然后做 视频 + 文字的 双模态 或者 三模态
4. 考虑 五种人格维度有关联，用一个模型训而不是五个
4. 多模态 ： 早期融合和后期融合
